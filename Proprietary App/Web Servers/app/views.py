...
def search():
    ...
    result = main.get_select(f"SELECT * FROM book_extended WHERE s_title = '{text}'")
    print(request)
    print(result)
...
@app.route('/book')
def book():
    result = main.get_select("SELECT * FROM book_extended")
    print(request)
    print(result)
    if isinstance(result, DataFrame):
        return render_template("includes/table.html", column_names=result.columns.values,
...
def execute_add_book():
    ...
    result = main.add_new_book(new_book)
    if result is True:
...
def profile():
    ...
    if active_user_id is not None:
        count_read_books = main.get_select(f"""  SELECT COUNT(n_borrow_item_id) 
                                                FROM loan 
                                                    LEFT JOIN borrow_item as bi on loan.n_loan_id = bi.n_loan_id
                                                WHERE n_user_id = {active_user_id};""").iat[0, 0]

        user_info = main.get_select(f""" SELECT 
                                            s_user_name, 
                                            s_first_name, 
                                            s_last_name, 
                                            dt_date_of_birth, 
...
def return_book_by_isbn():
    if request.method == 'POST':
        book_id =main.get_book_id_by_isbn(request.form["book_isbn"])
        result = main.return_book(book_id)
        if result is True:
...
def return_book_by_title():
    if request.method == 'POST':
        ...
        if edition == "":
            edition = 1
        book_id = main.get_book_id_by_title_edition(request.form["book_title"], edition)
        result = main.return_book(book_id)
        if result is True:
            return render_template("includes/success.html", title='Book Returned',
                                   text='You have returned the book successfully.')
...
def logged_in():
    session['user_id'] = main.set_user(request.form['user_name'], request.form['password'])
...
def active_loans():
    result = main.get_select(Selections.sql_user_active_loans(session.get('user_id', None)))
    print(request)
    if isinstance(result, DataFrame):
        return render_template("includes/table.html", column_names=result.columns.values,
                               row_data=list(result.values.tolist()),
...
def loan_history():
    result = main.get_select(Selections.sql_user_loan_history(session.get('user_id', None)))
    print(request)
    if isinstance(result, DataFrame):
        return render_template("includes/table.html", column_names=result.columns.values,
                               row_data=list(result.values.tolist()),
